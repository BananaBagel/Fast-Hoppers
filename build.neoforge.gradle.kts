...
neoForge {
	...
	runs {
		...
		// Headless GameTest run. "gameTestServer" is a run type defined by NeoForge's own userdev
		// config rather than by moddev, so there is no gameTest() helper to match client()/server().
		register("gametest") {
			type = "gameTestServer"
			gameDirectory = file("run/gametest")
			ideName = "NeoForge Game Test (${stonecutter.current.version})"
		}
	}
